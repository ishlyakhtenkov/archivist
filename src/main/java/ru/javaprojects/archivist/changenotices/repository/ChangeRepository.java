...
public interface ChangeRepository extends BaseRepository<Change> {
    ...
    @Query("SELECT c FROM Change c JOIN FETCH c.document WHERE UPPER(c.changeNotice.name) =UPPER(:changeNoticeName) " +
            "AND c.changeNotice.autoGenerated = TRUE ORDER BY c.document.decimalNumber")
    List<Change> findAllByAutogeneratedChangeNoticeName(String changeNoticeName);
