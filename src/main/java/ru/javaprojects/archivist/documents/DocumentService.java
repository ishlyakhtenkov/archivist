...
public class DocumentService {
    ...
    public Document getWithOriginalHolderAndDeveloper(long id) {
    ...
    private Invoice getAndCheckInvoice(BaseSendingToExtra sendingTo) {
    ...
    @Transactional
    public GroupOperationResult deleteGroupSending(GroupDeleteSendingTo groupDeleteSendingTo) {
        ...
        subscriberRepository.saveAll(subscribers);
        List<String> processedDocuments = forDeleteSendings.stream()
                .map(sending -> sending.getDocument().getDecimalNumber())
                .peek(decimalNumbers::remove)
                .toList();
        return new GroupOperationResult(processedDocuments, decimalNumbers);
    }

    @Transactional
    public GroupOperationResult unsubscribeGroup(GroupUnsubscribeTo groupUnsubscribeTo) {
        Assert.notNull(groupUnsubscribeTo, "groupUnsubscribeTo must not be null");
        Consumer<Subscriber> unsubscribeOperation = subscriber -> {
            subscriber.setSubscribed(false);
            subscriber.setUnsubscribeReason(groupUnsubscribeTo.getUnsubscribeReason());
            subscriber.setUnsubscribeTimestamp(LocalDateTime.now());
        };
        return subscribeGroup(groupUnsubscribeTo.getCompanyId(), groupUnsubscribeTo.getFile(), unsubscribeOperation);
    }

    @Transactional
    public GroupOperationResult resubscribeGroup(GroupSubscribeTo groupSubscribeTo) {
        Assert.notNull(groupSubscribeTo, "groupSubscribeTo must not be null");
        Consumer<Subscriber> resubscribeOperation = subscriber -> {
            subscriber.setSubscribed(true);
            subscriber.setUnsubscribeReason(null);
            subscriber.setUnsubscribeTimestamp(null);
        };
        return subscribeGroup(groupSubscribeTo.getCompanyId(), groupSubscribeTo.getFile(), resubscribeOperation);
    }

    public GroupOperationResult subscribeGroup(Long companyId, MultipartFile decimalNumbersFile,
                                               Consumer<Subscriber> subscribeOperation) {
        companyService.get(companyId);
        List<String> decimalNumbers = new ArrayList<>(FileUtil.readAllLines(decimalNumbersFile, true));
        List<Subscriber> subscribers = subscriberRepository
                .findAllByCompanyIdAndDocumentDecimalNumbers(companyId, decimalNumbers);
        subscribers.forEach(subscribeOperation);
        subscriberRepository.saveAll(subscribers);
        List<String> processedDocuments = subscribers.stream()
