...
public class DocumentService {
    ...
    @Transactional
    public GroupAddSendingResult createGroupSending(GroupSendingTo groupSendingTo) {
        Assert.notNull(groupSendingTo, "groupSendingTo must not be null");
        List<String> decimalNumbers = new ArrayList<>(FileUtil.readAllLines(groupSendingTo.getFile(), true));
        ...
        if (!documents.isEmpty()) {
            ...
            Invoice invoice = getAndCheckInvoice(groupSendingTo);
            ...
            List<Subscriber> subscribers = subscriberRepository.findAllByCompany_IdAndDocument_IdIn(groupSendingTo.getCompanyId(),
            ...
            Company company = companyService.get(groupSendingTo.getCompanyId());
            List<Subscriber> newSubscribers = documents.stream()
                    ...
                    .map(document -> new Subscriber(null, document, company,
                            groupSendingTo.getStatus() != UNACCOUNTED_COPY, groupSendingTo.getStatus()))
                    .toList();
            subscriberRepository.saveAll(newSubscribers);
            subscribers.forEach(subscriber -> updateSubscriberIfNecessary(subscriber, groupSendingTo));
