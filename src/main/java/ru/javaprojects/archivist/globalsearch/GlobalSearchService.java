...
public class GlobalSearchService {
    private final String query = """
                ...
                FROM documents d WHERE UPPER(d.decimal_number) LIKE UPPER(CONCAT('%', ?1, '%')) 
                AND d.auto_generated IS FALSE LIMIT ?2) 
                UNION
                (SELECT CONCAT(d.decimal_number, ' document card') AS header, 'documents' AS group_name, 
                d.id AS entity_id, CONCAT('name = ', d.name) AS match_value 
                FROM documents d WHERE UPPER(d.name) LIKE UPPER(CONCAT('%', ?1, '%')) 
                AND d.auto_generated IS FALSE LIMIT ?2) 
                UNION
                (SELECT CONCAT(d.decimal_number, ' document card') AS header, 'documents' AS group_name, 
                d.id AS entity_id, CONCAT('inventory number = ', d.inventory_number) AS match_value 
                FROM documents d WHERE UPPER(d.inventory_number) LIKE UPPER(CONCAT('%', ?1, '%')) 
                AND d.auto_generated IS FALSE LIMIT ?2) 
                UNION
                ...
                FROM change_notices c WHERE UPPER(c.name) LIKE UPPER(CONCAT('%', ?1, '%')) 
                AND c.auto_generated IS FALSE LIMIT ?2) 
                UNION
                ...
                WHERE UPPER(d.decimal_number) LIKE UPPER(CONCAT('%', ?1, '%')) LIMIT ?2)
                UNION
                (SELECT CONCAT(c.name, ' company card') AS header, 'companies' AS group_name, 
                c.id AS entity_id, CONCAT('name = ', c.name) AS match_value 
                FROM companies c WHERE UPPER(c.name) LIKE UPPER(CONCAT('%', ?1, '%')) LIMIT ?2)
                UNION
                (SELECT CONCAT(d.name, ' department card') AS header, 'departments' AS group_name, 
                d.id AS entity_id, CONCAT('name = ', d.name) AS match_value 
                FROM departments d WHERE UPPER(d.name) LIKE UPPER(CONCAT('%', ?1, '%')) LIMIT ?2)
                LIMIT ?2
                """;
    ...
    public List<SearchResult> getSearchResults(String keyword) {
        return jdbcTemplate.query(query, BeanPropertyRowMapper.newInstance(SearchResult.class), keyword, searchResultsLimit);
    }
}
