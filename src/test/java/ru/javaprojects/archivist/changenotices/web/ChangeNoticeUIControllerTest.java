...
class ChangeNoticeUIControllerTest extends AbstractControllerTest implements TestContentManager {
    ...
    void getAutoGenerated() throws Exception {
        perform(MockMvcRequestBuilders.get(CHANGE_NOTICES_URL_SLASH + CHANGE_NOTICE_3_ID))
                .andExpect(exception().exceptionPage("Entity with id=" + CHANGE_NOTICE_3_ID + " not found", NotFoundException.class));
    }
    ...
    void showEditFormAutoGenerated() throws Exception {
        perform(MockMvcRequestBuilders.get(CHANGE_NOTICES_EDIT_FORM_URL + CHANGE_NOTICE_3_ID))
                .andExpect(exception().exceptionPage("Entity with id=" + CHANGE_NOTICE_3_ID + " not found", NotFoundException.class));
    }
